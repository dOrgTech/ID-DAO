...
contract('Registry', (accounts) => {
  ...
  describe('Functions', async () => {
    ...
    it('add', async () => {
      ...
      const res: object = await instances.Registry.add(users[0].address, users[0].metadata, {from: users[0].address});
      assert.ok(res);

      // Check if added
      const metadata: string = await instances.Registry.registry.call(users[0].address);
    ...
    it('remove', async () => {
      // Add an ID to the Registry
      let res: object = await instances.Registry.add(users[0].address, users[0].metadata, {from: users[0].address});
      assert.ok(res);

      // Check if existing
      let metadata: string = await instances.Registry.registry.call(users[0].address);
      assert.ok(metadata, 'error, metadata is: ' + metadata);
